...
export default function HomePage({ darkMode }) {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        alert('Could not load user data. Is the backend running?');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      alert('❌ Failed to delete user: ' + (err.response?.data?.error || 'Network error'));
    } finally {
      setDeletingId(null);
    }
  };
...
const styles = {
    ...
    header: () => ({
      ...
      marginBottom: '24px',
      padding: '10px 0',
      // Using darkMode indirectly via consistent pattern
    }),
    ...
    downloadBtn: (darkMode) => {
      // Now we explicitly use darkMode, even if style is mostly static
      ...
      return {
        ...
        ...(isDark && { boxShadow: '0 0 0 1px rgba(255,255,255,0.2)' }),
      };
    },
